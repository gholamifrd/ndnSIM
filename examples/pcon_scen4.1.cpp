...
main(int argc, char* argv[])
{
  ...
  const int RUNTIME = SimHelper::getEnvVariable("RUNTIME", 120);
  ...
  // Set to "EQUAL", "DIFF_DELAY", or "DIFF_BW"
  const std::string SCENARIO = SimHelper::getEnvVariableStr("SCENARIO", "DIFF_DELAY");

  assert(SCENARIO == "EQUAL" || SCENARIO == "DIFF_DELAY" || SCENARIO == "DIFF_BW");

  // You may also want to edit enviroment variable "CHANGE_PER_MARK",
  ...
  topologyReader.SetFileName(
      "src/ndnSIM/examples/topologies/topo-pcon_scen4.1_" + SCENARIO + ".txt");
  topologyReader.Read();
  ...
  for (ns3::Ptr<ns3::Node> n : consumers) {
    consumerHelper1.Install(n);
  }
