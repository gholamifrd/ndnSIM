...
{
  const std::string SCEN_NAME = SimHelper::getEnvVariableStr("SCEN_NAME", "PCON_2");
  const int RUNTIME = SimHelper::getEnvVariable("RUNTIME", 15);
  const uint32_t QUEUE_SIZE_PKTS = SimHelper::getEnvVariable("QUEUE_SIZE_PKTS", 200);
  ...
  ndn::AppHelper consumerHelper2 { consumerHelper1 };
  consumerHelper2.SetAttribute("StartTime", StringValue("5s"));
  consumerHelper2.Install(consumers[1]);
